...
from utils import extract_video_id_from_url, get_youtube_transcript, clean_script, evaluate, split_into_sentences

if __name__ == "__main__":
    st.set_page_config(page_title="YouTube 받아쓰기 앱", layout="centered")
    st.title("📚 YouTube 영어 받아쓰기 앱")

    st.write("유튜브 영상의 스크립트를 가져와 문장 단위로 받아쓰기 연습을 할 수 있습니다.")

    youtube_url = st.text_input("유튜브 영상 URL을 입력하세요:", help="예: https://www.youtube.com/watch?v=M-y14-3Y6gE")
        ...
        if youtube_url:
            ...
            if video_id:
                ...
                if original_script:
                    ...
                    st.success(f"총 {st.session_state.total_sentences}개의 문장을 찾았습니다. 받아쓰기를 시작하세요!")
                else:
                    st.warning("영상의 스크립트를 가져오는 데 실패했습니다. URL을 확인하거나 다른 영상을 시도해주세요.")
            else:
            ...
            if st.button("채점하기", use_container_width=True):
                if user_input:
                    with st.spinner("Gemini AI가 채점 중입니다..."):
                        scoring_result = evaluate(st.session_state.sentences[idx], user_input)
        ...
        if st.session_state.scores[idx]:
            with st.expander("채점 결과 보기", expanded=True):
                ...
                if "error" in scoring_results:
                    ...
                    st.write(f"**👍 잘한 점:** {scoring_results.get('positive_feedback')}")
